...
3. **二重起動ロック**: `start-run.mjs` は `runs/.lock`(日付に依存しない固定パス)を
   排他的に作成してから`runs/$TODAY.md`を書く。**別のバッチが実行中でロックが取れなかった場合、
   このコマンドは vault に何も書かずにエラーで終了する**(エラーメッセージにロックの
   pid・host・date・開始時刻が出る)。
   その場合は **`_inbox/`の写真も含め vault を一切変更せず、そのまま終了すること**
   (`finish-run.mjs`も呼ばない。ロックは相手のランのものなので解放してはならない)。
   標準出力には「別のランが実行中のため何もせず終了した」旨とロックの保持者情報を出す。
   なお2時間を超えた古いロック(クラッシュ跡)は`start-run.mjs`が警告のうえ自動的に奪うので、
   その場合はそのまま処理を続行してよい。
...
2. **決定的アダプタによる先行取り込み**（環境変数 `STUDY_AI_ATTEMPTS=1` のときのみ）:
   各エントリについて `node analysis/helpers/ingest-artifact.mjs "<relPath>"` を実行する。
   返り値の `adapter` が `null` 以外なら、そのエントリは設問レベルで
   `vault/data/attempts.jsonl` に取り込まれている。この場合、次の項目3・4の
   LLMによる読み取りをスキップしてよい（正誤・単元・所要時間が機械的に確定しているため）。

   - `adapter` が `null` の場合は、従来どおり項目3以降のLLM読み取りを行う。
   - `unknownMarks` が1以上の場合は、色判定に失敗した行があるため、
     **その原本を要確認TODOに回す**（黙って正解扱いにしない）。
   - コマンドがエラーで終了した場合も、そのエントリを要確認TODOに回し、バッチ全体は止めない。

   `STUDY_AI_ATTEMPTS` が未設定の場合、この項目全体をスキップし、
   従来と完全に同じ処理を行う。
3. 決定的アダプタで処理されなかった各エントリについて、画像を読み込むツール(Claude Codeでは `Read` ツール、
   Codex CLIでは組み込みの`view_image`/PDF読み取りツール)でファイルを直接読む
   (`STUDY_AI_VAULT_DIR`配下のローカルファイルなので、パスをそのまま渡せばよい)。
4. **読み取り規約**(親スペック「夜間バッチの新フロー」2.を具体化):
...
1. **Vaultミラー同期**: `node analysis/helpers/sync-vault-to-supabase.mjs` を実行し、`vault/`配下の
   ...
   **このコマンドが失敗しても後続の項目(手順7の2〜4)を止めない。** 手順1〜6は既にvaultへの書き込みを
   完了しているため、同期の失敗はデータ損失にならない。失敗した場合はエラー内容を控えておき、
   手順7の2の`run-summary.json`の`lines`に`"Vaultミラー同期に失敗しました(次回再試行): <エラー内容>"`
...
3. `node analysis/helpers/finish-run.mjs "$TODAY" ok analysis/tmp/run-summary.json`
   ...
   このコマンドは完了報告の書き込みと同時に、手順1で取得した`runs/.lock`を解放する
   (書き込みが失敗した場合でも必ず解放される)。
4. **Vaultミラー同期(2回目・仕上げ)**: `node analysis/helpers/sync-vault-to-supabase.mjs` を
   もう一度実行する。理由: 1の同期は`run-summary.json`に件数を載せるために完了報告より前に
   走る必要があるが、そのままだとミラー上の`runs/$TODAY.md`が「実行中」のまま常に1回分古くなる。
   3の直後にもう一度同期することで、最終的な完了報告もミラーに載る。
   **この2回目が失敗しても`error`にせず、後続の項目も止めない**(vault本体は既に正しく、
   次回実行時に差分がまとめて同期される)。失敗した場合は5の標準出力にその旨を記す。
   `run-summary.json`には1回目の件数が載っているので、2回目の件数は載せなくてよい。
5. 標準出力(実行ログ)に、以下を簡潔にまとめて出力して終了する:
...
- **ロックの後始末**: 手順1でランを開始できた(ロックを取れた)場合、途中でどんなエラーが
  起きても最後に必ず `node analysis/helpers/finish-run.mjs "$TODAY" error <summaryJSON>` を
  実行して`runs/.lock`を解放してから終了すること(解放しないと次回以降のランが2時間止まる)。
  逆に、**手順1でロックが取れずに弾かれた場合は`finish-run.mjs`を呼ばない**
  (他のランのロックを奪うことになる)。何も書かずに終了する。
  クラッシュ等でロックが残ってしまったときの手動解除は
