...
**移行日を文面に入れない理由:** 実際の移行実行日は Task 8 を手動で走らせるまで確定しない
(本計画では実データ移行を行わない)。当初案は `2026-07-26` という仮の日付を埋めて
「実装時に書き換える」としていたが、**書き換え忘れると画面に誤った日付が出続ける**。
日付を持たない文面にすれば、いつ移行しても正しいまま。日付を出したくなったら、移行実行後に
別途1行変更すればよい(`/stats` はこのTaskでロジックを変更しないため、既存の `Alert` import を
そのまま使う。追加の import は不要)。
