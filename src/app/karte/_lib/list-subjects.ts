...
export async function listKarteSubjects(): Promise<string[]> {
  ...
  try {
    subjectEntries = await readdir(path.join(root, "subjects"), { withFileTypes: true });
  } catch (error) {
  ...
  try {
    derivedEntries = await readdir(path.join(root, "data", "derived"), { withFileTypes: true });
  } catch (error) {
  ...
  for (const entry of derivedEntries) {
    const match = entry.isFile() ? /^skills-(.+)\.json$/.exec(entry.name) : null;
    if (match) names.add(match[1]);
  }
  return Array.from(names).sort();
}
