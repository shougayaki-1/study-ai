...
describe("listKarteSubjects", () => {
  ...
  beforeEach(async () => {
    ...
    await mkdir(path.join(vaultDir, "subjects", "日本史"), {
      recursive: true,
    });
    await mkdir(path.join(vaultDir, "subjects", "世界史"), {
      recursive: true,
    });
    await mkdir(path.join(vaultDir, "data", "derived"), { recursive: true });
    await writeFile(
      path.join(vaultDir, "data", "derived", "skills-化学基礎.json"),
      "{}",
      "utf-8",
    );
    await writeFile(
      path.join(vaultDir, "subjects", "not-a-subject.md"),
      "stray file",
      "utf-8",
    );
...
describe("listKarteSubjectsFromSupabase", () => {
  it("extracts unique subject directory names from subjects/ prefixed paths", async () => {
    ...
    expect(await listKarteSubjectsFromSupabase(client)).toEqual([
      "世界史",
      "化学基礎",
      "日本史",
    ]);
